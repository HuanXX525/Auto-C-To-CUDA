...
__global__ void _auto_kernel_1(float A[5000][5000],float B[5000][5000],float C[5000][5000])
{
...
__global__ void _auto_kernel_0(float A[5000][5000],float B[5000][5000])
{
  int thread_x_id;thread_x_id = blockIdx.x * blockDim.x + threadIdx.x;
  int thread_y_id;thread_y_id = blockIdx.y * blockDim.y + threadIdx.y;
  if (thread_x_id && thread_y_id) 
    if (thread_x_id <= 5000 && thread_y_id <= 5000) {
      A[1 * thread_x_id + -1][1 * thread_y_id + -1] = ((float )(sin((double )(1 * thread_x_id + -1 + (1 * thread_y_id + -1))) * sin((double )(1 * thread_x_id + -1 + (1 * thread_y_id + -1)))));
      B[1 * thread_x_id + -1][1 * thread_y_id + -1] = ((float )(cos((double )(1 * thread_x_id + -1 + (1 * thread_y_id + -1))) * cos((double )(1 * thread_x_id + -1 + (1 * thread_y_id + -1)))));
    }
}

...
bool NestCollectPass::transform(SgProject *project, PassContext &ctx)
{
    ...
    // 使用前面 FuncsCollectPass 收集的函数列表，函数内重新查询 for 循环
    // （不能用 CollectForLoopsPass 的 for_loops：它在 InlinePass 之前收集，
    //  且本 pass 的 imperfect→perfect 转换会替换循环节点，旧指针会失效）
    ...
    for (auto funcIter = functions.begin(); funcIter != functions.end(); funcIter++)
    {
        ...
        SgFunctionDefinition *defn =
            isSgFunctionDefinition((*funcIter)->get_definition());
        SgSourceFile *sourceFile =
            SageInterface::getEnclosingNode<SgSourceFile>(defn);
        SgGlobal *fileGlobalScope = sourceFile ? sourceFile->get_globalScope() : nullptr;
    ...
    // 登记合格嵌套，供后续 SSA / CodeGen 阶段消费
    ctx.set<std::vector<QualifiedNest>>("qualified", std::move(qualified));
